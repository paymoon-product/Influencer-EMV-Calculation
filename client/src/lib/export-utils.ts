...
function generateMultiplePDFHTML(calculations: EmvCalculation[]): string {
  ...
  return `
    ...
    <body>
      ...
      <div class="footer">
        <div>This report was generated by the Aspire EMV Calculator</div>
        <div>Report Period: ${calculations.length > 0 ? new Date(calculations[calculations.length - 1].date).toLocaleDateString() : 'N/A'} - ${calculations.length > 0 ? new Date(calculations[0].date).toLocaleDateString() : 'N/A'} | Generated: ${new Date().toLocaleString()}</div>
      </div>
    </body>
    </html>
  `;
